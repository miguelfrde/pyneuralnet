...
def random_matrix(rows, cols, e1, e2):
	"""Create a random numpy array with values between e1 and e2"""
...
class NeuralNetwork:
	...
	def cost_function(self, theta, X, Y):
		...
		Y: matrix with expected output data (output_size x number_of_training_examples)
		Returns:
		J: cost funnction.
		...
		self.forward(X)
		J = - np.sum(Y * np.log(self.ao) + (1 - Y) * np.log(1 - self.ao)) / float(m)
		gradient_ih = np.zeros((self.hidden_size - 1, self.input_size))
		gradient_ho = np.zeros((self.output_size, self.hidden_size))
		deltak = self.ao - Y
		deltaj = np.dot(self.weights_ho.T[1:, :], deltak) * self.dsigmoid(self.ah[1:, :])
	...
	def update_weights(self, theta):
		"""
		Separate the weights in theta into weights from Input to Hidden Layer and
		weights from Hidden to Output Layer.
		"""
		self.weights_ih = theta[0:(self.input_size * (self.hidden_size - 1))]
		self.weights_ih = self.weights_ih.reshape(self.hidden_size - 1, self.input_size)
		self.weights_ho = theta[(self.input_size * (self.hidden_size - 1)):]
		self.weights_ho = self.weights_ho.reshape(self.output_size, self.hidden_size)

	def mse(self, Y):
		"""Mean Squared Error"""
		m = Y.shape[1]
		#norm = sum(np.apply_along_axis(np.linalg.norm, 0, (self.ao - Y).T))
		norm = sum(sum(abs(self.ao - Y) ** 2))
		return norm / (2 * m)

	def minimize(self, X, Y, lim_iters, disp=False, method='BFGS'):
		""" Use CG, BFGS or L-BFGS-B algorithms to minimize the cost function J."""
		return optimize.minimize(lambda t: self.cost_function(t, X, Y),
					x0 = np.append(self.weights_ih, self.weights_ho),
	...
	def train_and_errorplot(self, X, Y, lim_iters=1000, save=True, error='both'):
		...
		if error in ['both', 'j']:   plt.plot(x, yj)
		if error in ['both', 'mse']: plt.plot(x, ymse)
		plt.xlabel('Iterations')
		lbl = '$J(\\theta)$' if error == 'j' else \
			  'MSE'if error == 'mse' else '$J(\\theta)$ and $MSE$'
		plt.ylabel('Error ' + lbl)
		plt.show()
		if not save: self.update_weights(original_theta)
	...
	def train_from_files(self, x_file, y_file,
		...
		else: self.train(X, Y, lim_iters, disp)
	...
	def test(self, inputs, outputs, disp=True):
		...
		if disp: print 'Accuracy: %.2f%%' % (accuracy * 100)
		return accuracy
